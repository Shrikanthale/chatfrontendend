...
import useSendMessage from '../../Hooks/useSendMessage.js'; // <-- import the hook

const MessageInput = ({ isDarkMode = false }) => {
  const [message, setMessage] = useState('');
  const [isTyping, setIsTyping] = useState(false);
  const fileInputRef = useRef(null);

  const { sendMessage, loading } = useSendMessage(); // <-- get API function
  ...
  const handleSubmit = async (e) => {
    ...
    if (message.trim()) {
      console.log("inside2")
      await sendMessage(message); // <-- call API
      console.log("inside3")
      setMessage('');
      setIsTyping(false);
    }
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      // Later you can call sendMessage with file data
    }
  };

  return (
    <div
      className={`border-t p-3 lg:p-4 ${
